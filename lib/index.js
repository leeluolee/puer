...
(function() {
  ...
  processOptions = function(options) {
    return helper.merge(options, {
      ...
      addon: null,
      proxy: null,
      context: '/api/' // 代理的 context, 注意以斜杠结尾, 否则会匹配到 /api123 这样的路径
    });
  };

  function turnOnProxy(app, options) {
    ...
    // 只要在命令行使用了 rewrite 参数, 则说明需要开启 pathRewrite 功能
    if (options.hasOwnProperty('rewrite')) {
      // https://webpack.js.org/configuration/dev-server/#devserver-proxy
      // If you don't want /api to be passed along, we need to rewrite the path:
      // {'^/api/': ''}
      proxyOption.pathRewrite['^' + options.context] = options.rewrite ? options.rewrite : '';
    }

    app.use(proxy(options.context, proxyOption));
  }
